...
def cmd_scan(client: SnifferClient, args: argparse.Namespace) -> None:
    ...
    try:
        ...
        # drain remaining
        while client.frames:
            print_frame(client.frames.popleft())
            count += 1
        print(f"\nStopped. {count} frames captured, ~{client.dropped} dropped.")
...
def main() -> int:
    ...
    p_scan.add_argument("-c", "--channel", type=int, default=None,
    ...
    p_promisc.add_argument("action", nargs="?", choices=["on", "off"],
                           help="on/off (omit to query current status)")

    args = parser.parse_args()

    try:
        client = SnifferClient(args.port, baudrate=args.baud)
